...
import {
  bankReceiptOcrSchema,
  type BankReceiptOcr,
} from "../schemas/bank-receipt";
import {
...
export async function parseBankReceipt(opts: {
  buffer: Buffer;
  mimeType: string;
}): Promise<{ raw: unknown; parsed: BankReceiptOcr; rawText: string }> {
  ...
  const response = await client.messages.create({
    model: OCR_MODEL,
    max_tokens: 1024,
    system: BANK_RECEIPT_SYSTEM_PROMPT,
    messages: [
      {
        role: "user",
        // eslint-disable-next-line @typescript-eslint/no-explicit-any
        content: [sourceBlock as any, { type: "text", text: BANK_RECEIPT_USER_PROMPT }],
      },
    ],
  });
